...
if not DATASET_PATH.exists():
    print(f"Sample file not found. Checked: {', '.join(str(path) for path in DATASET_CANDIDATES)}")
    print(f"   Please run: python {Path(__file__).name} from the project root")
    print("   Or generate the sample with: python scripts/extract_sample.py")
    sys.exit(1)
...
def safe_preview(value: str, max_chars: int = 150) -> str:
...
def main() -> None:
    ...
    for i, (q, a) in enumerate(qa_pairs[:2]):
        print(f"\nPair {i+1}:")
        print(f"  Q: {safe_preview(q, 150)}...")
        print(f"  A: {safe_preview(a, 150)}...")
    ...
    if results["hit_rate"] > 0.10:
        ...
        print("Your hit rate is lower than expected.")
